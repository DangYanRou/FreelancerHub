...
const handleWorkplaceChange = (event) => setWorkplaceInput(event.target.value);
...
const handleCurrencyInputChange = (event) => setCurrencyInput(event.target.value);


// Check if all required fields are filled
const allFieldsFilled = projectName !== '' && minInput !== '' && maxInput !== '' && workplaceInput !== '' && locationInput !=='';
...
const handleFormSubmit = (event) => {
  ...
  if (!workplaceInput) {
    setWorkplaceError('Workload is required');
  } else {
    setWorkplaceError('');
  }
  ...
  if (projectName && min && max && workplaceInput && locationInput && currencyInput) {
...
const handleButtonClick = (event) => {
  ...
  return (
    <div className="flex flex-col items-start justify-center">
        ...
        <Heading as="h1" className="ml-[20px] tracking-[-0.90px] py-5 md:p-5">
            Create Project
          </Heading>
  ...
  <div className="w-1/2 mr-8">
    ...
    <select className="flex h-[40px] w-full items-center justify-center self-stretch rounded-[10px] border border-solid border-gray-500 bg-white-A700 px-5"
     id="workplace" value={workplaceInput} onChange={handleWorkplaceChange}>
    ...
    {workplaceError && <p className="text-red-500 text-xs italic">{workplaceError}</p>}
...
<div className="w-3/4 m-4 flex">
 ...
 id="min" type="number" min="0"  value={minInput} onChange={handleMinInputChange}/>
      {minError && <p className="text-red-500 text-xs italic">{minError}</p>}
  </div>
  <div className="flex items-center w-1/3 mr-2">
 ...
 id="max" type="number" min="0"  value={maxInput} onChange={handleMaxInputChange} />
      {maxError && <p className="text-red-500 text-xs italic"> {maxError}</p>}
  ...
  value={currencyInput} onChange={handleCurrencyInputChange}>
  ...
  {currencyError && <p className="text-red-500 text-xs italic">{currencyError}</p>}
